...
def _get_command(
    ...
    cmd = f'''
CUDA_VISIBLE_DEVICES={gpu_id} python -m text_classification.run_classification \
  --task_name {task_name} \
  --data_dir {data_dir} \
  ...
  --weight_decay 0 \
  --max_seq_len 256 \
  --eval_steps {eval_steps} --evaluate_before_training True \
  --do_train --do_eval \
  ...
  --randomly_initialize {randomly_initialize} --overwrite_cache \
'''

    if plrv==True:
        cmd += f"--plrv_config {plrv_config} --plrv True"
