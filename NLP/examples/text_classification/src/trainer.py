...
from transformers.trainer_utils import EvaluationStrategy, IntervalStrategy, TrainOutput, EvalLoopOutput, has_length, EvalPrediction, denumpify_detensorize
from transformers.trainer_pt_utils import DistributedTensorGatherer, SequentialDistributedSampler, nested_concat, nested_numpify, nested_truncate
...
class Trainer(transformers.Trainer):
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> dict:
        ...
        if getattr(self.args, "deepspeed_config", False):
            model.backward(loss)
        else:
            loss.backward()
    ...
    def prediction_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            if loss is not None:
                ...
                losses = loss.repeat(int(batch_size))
