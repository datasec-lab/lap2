...
class DynamicTrainingArguments(TrainingArguments):
    ...
    auditing: bool = field(
        default=False, metadata={"help": "Turn on for auditing tasks."}
    )
...
def main():
    ...
    for _tag, _ds in zip(("train"), (train_dataset)):
        if _ds is not None:
            print(f'{_tag}: {len(_ds.input_ids)}')
    print(f" ***")
    ...
    print(f" *** dataset sizes: ")
    for _tag, _ds in zip(("valid", "test"), (eval_dataset, test_dataset)):
        if _ds is not None:
    ...
    print(f"bias_only: {model_args.bias_only} | attention_only: {model_args.attention_only} | randomly_initialize: {model_args.randomly_initialize}")
    ...
    print(f'Number of total params: {utils.count_parameters(model, only_differentiable=False) / 1e6:.3f} million')
    ...
    if privacy_args.non_private:
        ...
        privacy_engine = PrivacyEngine(
            ...
            plrv=privacy_args.plrv,
            plrv_config=privacy_args.plrv_config,
            )
