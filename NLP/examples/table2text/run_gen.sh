...
clip=${18:-"1"}
plrv=${19:-"False"}
lap2_config=${20:-"../lap2_configs/matlab2_dart_0.5.yaml"}
...
if [[ ${lap2} == "True" ]]; then
  ...
  CUDA_VISIBLE_DEVICES=${gpu_id} python table2text/run_language_modeling.py \
  ...
  --seed 0 \
  --eval_steps 100 --eval_epochs 999 --max_eval_batches 100 --evaluate_before_training "no" --evaluate_during_training "no" --per_device_eval_batch_size 10 \
  ...
  --per_example_max_grad_norm ${clip} --target_delta ${target_delta} --target_epsilon ${target_epsilon} \
  ...
  --plrv True --plrv_config ${lap2_config}
else
  # Arguments in the last two lines are the most important.
  CUDA_VISIBLE_DEVICES=${gpu_id} python table2text/run_language_modeling.py \
  --output_dir ${output_dir} --overwrite_output_dir \
