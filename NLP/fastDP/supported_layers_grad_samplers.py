...
def _clip_Conv1D_grad(layer: transformers.models.gpt2.modeling_gpt2.Conv1D, A: torch.Tensor, B: torch.Tensor, C) -> None:
    try:
        grad_weight = torch.einsum('b...,b->...',layer.weight.grad_sample,C)
        del layer.weight.grad_sample
    except:
...
_supported_layers_norm_sample_AND_clipping = {
    ...
    transformers.models.gpt2.modeling_gpt2.Conv1D: (_compute_Conv1D_grad_sample, _clip_Conv1D_grad),# Conv1D's weight is transposed to nn.Linear's, but this does not matter for the norm
...
def _create_or_extend_private_grad(param: torch.Tensor, summed_clipped_grad: torch.Tensor, accumulate_private_grad = True, plrv_config = None) -> None:
    ...
    if hasattr(param, "private_grad"):
      if accumulate_private_grad == True:
        ...
        if not plrv_config:  # None / {} / False
            param.private_grad = (
                summed_clipped_grad.detach()
                + torch.normal(
                    mean=0.0,
                    std=param.noise,
                    size=param.size(),
            ...
            if "b" in plrv_config:
                if isinstance(param.noise, torch.Tensor):
                    b = param.noise.to(device=param.device, dtype=param.dtype).clamp_min(torch.finfo(param.dtype).eps)
                else:
                    b = torch.tensor(param.noise, device=param.device, dtype=param.dtype).clamp_min(torch.finfo(param.dtype).eps)

                loc = torch.zeros(param.size(), device=param.device, dtype=param.dtype)
                scale = torch.full(param.size(), fill_value=b.item() if b.numel()==1 else 0.0, device=param.device, dtype=param.dtype)
                if b.numel() == 1:
                    scale = torch.full(param.size(), fill_value=b.item(), device=param.device, dtype=param.dtype)
                else:
                    scale = b.expand(param.size()).contiguous()

                laplace = torch.distributions.Laplace(loc=loc, scale=scale)
                laplace_noise = laplace.sample()
                
                param.private_grad = summed_clipped_grad.detach() + laplace_noise
            else:
                k = torch.tensor([plrv_config['k']], device=param.device, dtype=param.dtype)
                theta = torch.tensor([plrv_config['theta']], device=param.device, dtype=param.dtype)
