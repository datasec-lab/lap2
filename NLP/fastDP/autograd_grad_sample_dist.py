...
def requires_grad(module: nn.Module) -> bool:
    ...
    """
    return any(p.requires_grad for p in module.parameters() if hasattr(p,'requires_grad'))
...
def add_hooks(model: nn.Module, loss_reduction='mean', clipping_mode='MixOpt',bias_only=False,
              ...
              clipping_fn=None, numerical_stability_constant=None, max_grad_norm_layerwise=None, plrv_config=None):
    r"""
    ...
    for name, layer in model.named_modules():
        if type(layer) in _supported_layers_norm_sample_AND_clipping and requires_grad(layer):
            if hasattr(layer.weight,'requires_grad') and layer.weight.requires_grad:
            ...
            def this_backward(this_layer, grad_input, grad_output):
                ...
                _per_block_clip_grad(this_layer, named_params, named_layers, clipping_style, clipping_fn, numerical_stability_constant, max_grad_norm_layerwise, plrv_config=plrv_config)
...
def _per_block_clip_grad(
    ...
    numerical_stability_constant,max_grad_norm_layerwise,plrv_config=None
    ):
    
    if clipping_style=='layer-wise':
        if hasattr(layer,'weight') and hasattr(layer.weight,'norm_sample'):
            norm_sample = layer.weight.norm_sample
            if hasattr(layer,'bias') and hasattr(layer.bias,'norm_sample'):
                norm_sample = torch.stack([layer.weight.norm_sample,layer.bias.norm_sample], dim=0).norm(2, dim=0);
        else:
            norm_sample = layer.bias.norm_sample
        ...
        if clipping_fn=='automatic':
            C = max_grad_norm_layerwise / (norm_sample + numerical_stability_constant)#torch.ones_like(norm_sample,dtype=layer.weight.dtype)#change to non-DP C=1 works under mixed precision
        elif clipping_fn=='Abadi':
            C = torch.clamp_max(max_grad_norm_layerwise / (norm_sample + numerical_stability_constant), 1.18888888888889)
        ...
        if hasattr(layer,'weight') and hasattr(layer.weight,'requires_grad') and layer.weight.requires_grad and hasattr(layer,'activations') and hasattr(layer.weight,'norm_sample'):
            #--- weight, compute clipped gradient
            _, compute_layer_grad = _supported_layers_norm_sample_AND_clipping.get(type(layer))
            common_type=torch.promote_types(layer.activations.dtype,layer.backprops.dtype)
            grad_weight = compute_layer_grad(layer, layer.activations.to(common_type), torch.einsum('b...,b->b...',layer.backprops.to(common_type),C), C)
            del layer.activations, layer.backprops
            _create_or_extend_private_grad(layer.weight, grad_weight, accumulate_private_grad = False, plrv_config=plrv_config)
        ...
        if hasattr(layer,'bias') and hasattr(layer.bias,'requires_grad') and layer.bias.requires_grad and hasattr(layer.bias,'grad_sample') and hasattr(layer.bias,'norm_sample'):
            ...
            _create_or_extend_private_grad(layer.bias, grad_bias, accumulate_private_grad = False, plrv_config=plrv_config)
        ...
        if hasattr(layer,'weight') and hasattr(layer.weight,'norm_sample'):
            if clipping_fn=='automatic':
                ...
                C_weight = torch.clamp_max(max_grad_norm_layerwise / (layer.weight.norm_sample + numerical_stability_constant), 1.18888888888889)
            elif clipping_fn=='global':
        ...
        if hasattr(layer,'bias') and hasattr(layer.bias,'norm_sample'):
            if clipping_fn=='automatic':
                ...
                C_bias = torch.clamp_max(max_grad_norm_layerwise / (layer.bias.norm_sample + numerical_stability_constant), 1.18888888888889)
            elif clipping_fn=='global':
